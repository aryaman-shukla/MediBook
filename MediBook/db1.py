...
import stdiomask
conn=sql.connect(host='localhost',user='root',passwd='Shukla@1402')
if conn.is_connected():
    print('SUCCESSFULLY CONNECTED')
    c1=conn.cursor()
    c1.execute('create database if not exists hospital_db')
    c1.execute('use hospital_db')
    CH='yes'
    while CH.lower()=="yes":
        print()
        print()
        print("================================================================================")
        print()
        print("\t\t*_*_*_WELCOME TO Medibook_*_*_*")
...
n_department=%s',(dept.upper(),))
                                    ...
                                    print("Invalid input. Please refer the menu to specify choices")
                                ch2=input("Do you want to continue viewing?:[yes/no]")
                        elif choice==3:
                            ch3='yes'
                            while ch3.lower()=='yes':
                                print()
...
CONTACT NO.")
    ...
    print()
    print("! ! (^_^) Thank You for using Medibook(^_^) ! !")
